...
```{r}
conc_t1 <- readxl::read_excel("../concurrent_overview.xlsx",col_names = FALSE)
```
...
```{r}
panther <- read.csv("/Users/mondal0000/Documents/clean_pantherhuman.csv")
gene_id <- as.character(panther[panther$family_id %in% rownames(sum_t1),]$gene_symbol)
```
...
write.csv(go_df,"enrichGO_baseOutput_36Families_minusEmptyGene.csv")
```
...
write.csv(selected_terms_union,"Selected_GO_Terms_36Families.csv")
```
